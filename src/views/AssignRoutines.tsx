    ...
    <div className="space-y-4 md:space-y-6">
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 p-2 md:p-0">
        <div className="flex flex-col sm:flex-row sm:items-center gap-4 flex-1">
          <h1 className="text-xl font-black text-gray-200 tracking-tight shrink-0 text-center sm:text-left">Asignación de Rutinas</h1>
        </div>
        <div className="relative group flex-1 max-w-md w-full">
          ...
          <input
            ...
            className="block w-full pl-10 pr-4 py-2.5 bg-gray-800 border  border-gray-800 rounded-xl shadow-sm focus:ring-4 focus:ring-teal-500/10 focus:border-teal-500 outline-none transition-all font-bold text-gray-300 text-sm placeholder:text-gray-300"
            value={searchTerm}
      ...
      {loading ? (
        ...
        <div className="space-y-4">
          {/* VISTA DESKTOP (TABLA) */}
          <div className="hidden md:block bg-gray-800 rounded-[2.5rem] border  border-gray-800 shadow-xl shadow-gray-200/20 overflow-hidden">
            <table className="w-full text-left">
              <thead>
                <tr className="bg-gray-900/50 text-[10px] uppercase tracking-[0.2em] text-gray-400 font-black border-b border-gray-800">
                  <th className="px-8 py-5">Cliente</th>
                  <th className="px-8 py-5">Cedula</th>
              ...
              <tbody className="divide-y divide-gray-50">
                {filteredClients.map((client) => (
                  <tr key={client.id} className="hover:bg-teal-900/30/20 transition-all group">
                    <td className="px-8 py-6">
                      <div className="flex items-center gap-4">
                        <div className="bg-gray-700 size-12 rounded-2xl flex items-center justify-center text-gray-400 group-hover:bg-teal-600 group-hover:text-white transition-all shadow-inner shrink-0">
                          <FontAwesomeIcon icon={faUsers} className="size-5" />
                        </div>
                        <div className="min-w-0">
                          <span className="font-black text-gray-200 text-base block truncate">{client.name}</span>
                    ...
                    <td className="px-8 py-6">
                      <span className="font-bold text-gray-400 bg-gray-900 px-3 py-1 rounded-lg text-sm border  border-gray-800">
                        {client.cedula}
                      </span>
                    </td>
                    <td className="px-8 py-6">
                      {client.active_assignments && client.active_assignments.length > 0 ? (
                        <div className="flex flex-col gap-2.5">
                          {client.active_assignments.map((a: any) => (
                            <div key={a.id} className="flex items-center justify-between group/item bg-gray-900/50 p-2 rounded-xl border  border-gray-800/50 hover:border-teal-200 hover:bg-teal-900/30/30 transition-all">
                              <div className="flex items-center gap-3 min-w-0">
                                <span className="text-[9px] bg-gray-800 text-teal-700 px-2 py-0.5 rounded-md font-black border border-gray-800/50 uppercase tracking-tighter shrink-0 shadow-xs">
                                  {["", "Lun", "Mar", "Mié", "Jue", "Vie", "Sáb", "Dom"][a.day_of_week] || "Gral"}
                                </span>
                                <div className="flex flex-col min-w-0">
                                  <span className="text-xs text-gray-300 font-bold truncate leading-tight">{a.routine_name}</span>
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-xs text-gray-400 font-bold italic tracking-tight">Sin rutinas asignadas</span>
                        </div>
                      )}
                    </td>
                    <td className="px-8 py-6 text-right">
                      <button
                        ...
                        className="bg-teal-900/30 text-teal-600 px-5 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-teal-600 hover:text-white transition-all shadow-sm active:scale-95 border border-gray-700flex items-center gap-2 ml-auto"
                      >
          ...
          <div className="grid grid-cols-1 gap-4 md:hidden">
            {filteredClients.map((client) => (
              <div key={client.id} className="bg-gray-800 p-6 rounded-3xl border  border-gray-800 shadow-sm flex flex-col gap-4">
                <div className="flex justify-between items-start gap-3">
                  <div className="flex items-center gap-3 min-w-0">
                    <div className="size-12 rounded-2xl bg-teal-900/30 shrink-0 flex items-center justify-center text-teal-600 shadow-inner">
                      <FontAwesomeIcon icon={faUsers} className="text-lg" />
                    </div>
                    <div className="min-w-0">
                      <h3 className="font-black text-gray-200 truncate text-base">{client.name}</h3>
                  ...
                  <div className="flex flex-col items-end gap-1">
                    ...
                    {client.active_assignments && client.active_assignments.length > 0 ? (
                      <div className="flex flex-wrap justify-end gap-1 max-w-[120px]">
                        {client.active_assignments.map((a: any) => (
                          <span key={a.id} className="text-[8px] bg-teal-900/30 text-teal-700 px-1.5 py-0.5 rounded-md font-black border  border-teal-100">
                ...
                <div className="space-y-3">
                  ...
                  {client.active_assignments && client.active_assignments.length > 0 ? (
                    <div className="space-y-2">
                      {client.active_assignments.map((a: any) => (
                        <div key={a.id} className="flex items-center justify-between bg-gray-900 p-3 rounded-2xl border  border-gray-800">
                          <div className="flex items-center gap-3 min-w-0">
                            <span className="text-[9px] bg-gray-800 text-teal-700 px-2 py-1 rounded-lg font-black border border-gray-700  uppercase tracking-tighter shrink-0 shadow-xs">
                              {["", "L", "M", "X", "J", "V", "S", "D"][a.day_of_week] || "?"}
                            </span>
                            <div className="flex flex-col min-w-0">
                              <span className="text-sm text-gray-200 font-bold truncate leading-tight">{a.routine_name}</span>
                    ...
                    <div className="py-4 text-center bg-orange-50/30 rounded-2xl border border-gray-700 border-dashed">
          ...
          {filteredClients.length === 0 && (
            <div className="bg-gray-800 rounded-[2.5rem] border  border-gray-800 py-24 text-center">
      ...
      {showModal && selectedClient && (
        <div className="fixed inset-0 bg-gray-900/60 backdrop-blur-md z-60 flex items-center justify-center p-4 animate-in fade-in duration-300">
          <div className="bg-gray-800 rounded-[2.5rem] w-full max-w-xl overflow-hidden shadow-2xl relative">
            ...
            <div className="p-6 md:p-8 border-b border-gray-700 bg-linear-to-r from-teal-700 to-teal-600 text-white">
              <h2 className="text-2xl font-black flex items-center gap-3">
                <div className="bg-gray-800/20 p-2 rounded-xl backdrop-blur-sm">
            ...
            <form onSubmit={handleAssign} className="p-6 md:p-8 space-y-6">
              ...
              <div className="flex flex-col-reverse sm:flex-row gap-3 md:gap-4 pt-4">
                ...
                <button
                  ...
                  className="flex-1 px-4 py-4 rounded-2xl bg-teal-600 text-white text-xs font-black uppercase tracking-widest hover:bg-teal-700 transition-all shadow-lg shadow-teal-100 disabled:opacity-50 active:scale-95 flex items-center justify-center gap-2"
                >
