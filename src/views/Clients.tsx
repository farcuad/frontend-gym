...
import { faEdit, faTrashAlt, faUser, faIdCard, faPhone, faTimes, faSpinner, faPlus } from "@fortawesome/free-solid-svg-icons";
...
const EmployeeTable: React.FC = () => {
  ...
  const handleDelete = (name: string) => {
    MySwal.fire({
      title: '¿Estás seguro?',
      text: `Vas a eliminar a ${name}. ¡No podrás revertir esto!`,
      icon: 'warning',
        ...
        <table className="min-w-full bg-white text-sm">
          <thead>
            <tr className="border-b border-gray-100">
              <th className="px-6 py-4 text-left font-bold text-gray-400 uppercase tracking-wider text-[11px]">#</th>
              <th className="px-6 py-4 text-left font-bold text-gray-400 uppercase tracking-wider text-[11px]">Nombre</th>
              <th className="px-6 py-4 text-left font-bold text-gray-400 uppercase tracking-wider text-[11px]">Cédula</th>
              <th className="px-6 py-4 text-left font-bold text-gray-400 uppercase tracking-wider text-[11px]">Teléfono</th>
              <th className="px-6 py-4 text-center font-bold text-gray-400 uppercase tracking-wider text-[11px]">Acciones</th>
            </tr>
          </thead>
          ...
          <tbody className="divide-y divide-gray-50">
            {employees.map((person, index) => (
              <tr key={index} className="hover:bg-teal-50/30 transition-colors group">
                <td className="px-6 py-4 whitespace-nowrap text-gray-500 font-medium">{index + 1}</td>
                <td className="px-6 py-4 whitespace-nowrap">
                  <div className="font-bold text-gray-800">{person.name}</div>
                </td>
                <td className="px-6 py-4 whitespace-nowrap text-gray-600 font-medium">{person.cedula}</td>
                <td className="px-6 py-4 whitespace-nowrap text-gray-600 font-medium">{person.phone}</td>
      ...
      {isEditOpen && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-gray-900/40 backdrop-blur-md animate-in fade-in duration-300">
          <div className="bg-white rounded-[2rem] shadow-2xl w-full max-w-md p-8 relative animate-in zoom-in-95 duration-200">
            ...
            <div className="mb-8">
              <h3 className="text-2xl font-black text-gray-800 leading-tight">Editar Cliente</h3>
              <p className="text-sm text-gray-400 font-medium">Actualiza la información del miembro.</p>
            </div>
            ...
            <form className="space-y-5" onSubmit={(e) => e.preventDefault()}>
              <div className="space-y-4">
                ...
                <div className="relative">
                  ...
                  <input
                    type="text"
                    defaultValue={selectedClient?.cedula}
                    placeholder="Cédula"
                    className="w-full pl-11 pr-4 py-3.5 bg-gray-50 border border-gray-100 rounded-2xl focus:ring-2 focus:ring-teal-500/20 focus:border-teal-500 outline-none transition-all text-sm font-medium"
                  />
                </div>
      ...
      {isCreateOpen && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-gray-900/40 backdrop-blur-md animate-in fade-in duration-300">
          <div className="bg-white rounded-[2rem] shadow-2xl w-full max-w-md p-8 relative animate-in zoom-in-95 duration-200">
            ...
            <div className="mb-8">
              <h3 className="text-2xl font-black text-gray-800 leading-tight">Nuevo Cliente</h3>
              <p className="text-sm text-gray-400 font-medium">Ingresa los datos del nuevo miembro.</p>
            </div>
