  ...
  return (
    <header className="h-16 bg-gray-800 border-b border-gray-700 flex items-center justify-between px-6 shadow-sm">
      ...
      <div className="flex items-center gap-4">
        <button
          className="text-gray-400 hover:text-teal-600 lg:hidden p-2 hover:bg-gray-700 rounded-md transition-colors"
      ...
      <div className="flex items-center gap-4 relative">
        ...
        <div className="relative" ref={userDropdownRef}>
          <button
            className="cursor-pointer flex items-center gap-2 rounded-full border  border-gray-700 px-3 py-1.5 text-sm font-medium hover:shadow-md transition-shadow bg-gray-800"
            onClick={() => setIsDropdownOpen(!isDropdownOpen)}
          >
            <div className="w-6 h-6 rounded-full bg-teal-600 flex items-center justify-center text-white text-[10px]">
              <FontAwesomeIcon icon={faUser} />
            </div>
            <span className="hidden sm:block text-gray-300 ">{user?.name || "Usuario"}</span>
          </button>

          {isDropdownOpen && (
            <div className="absolute right-0 mt-3 w-48 rounded-xl bg-gray-800 shadow-2xl border  border-gray-800 py-2 z-50">
              <button
                onClick={handleLogout}
                className="cursor-pointer flex items-center gap-2 w-full px-4 py-2 hover:bg-red-900/30 text-red-600 text-left font-medium"
              >
